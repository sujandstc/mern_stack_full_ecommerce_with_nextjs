...
export const Navbar = () => {
	...
	return (
		<>
			<>
				<div className={`bg-black z-50 bg-opacity-60 fixed  h-full w-full top-0 duration-500 ease-in-out ${search ? 'translate-y-0' : '-translate-y-[100%]'} `}>
					<div className='absolute right-10 top-6 bg-white rounded-full p-2'>
						<IoMdClose onClick={() => setSearch(false)} className={` text-3xl lg:text-4xl`} />
					</div>
					<div className='grid place-items-center h-full'>
						<div className='flex flex-col lg:flex-row px-2 space-y-4 lg:space-y-0 lg:space-x-4'>
							<input value={searchData} onChange={(e) => setSearchData(e.target.value)} type='text' placeholder='Search for products' className='w-[90vw] lg:w-[25rem] p-4 rounded-md' />
